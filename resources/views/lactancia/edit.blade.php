...
@section('title','Lactancia - Actualizar')
@section('content')
    <form action="{{ route('lactancia.update', $lactancia->id) }}" method="POST">
        @csrf
        <div class="form-group">
            <label for="descripcion">Descripción</label>
            <textarea class="form-control" id="descripcion" name="descripcion" rows="3" required>{{ $lactancia->descripcion }}</textarea>
        </div>
        <div class="form-group" hidden>
            <input type="number" class="form-control" id="lactancia_id" name="lactancia_id" value="{{$lactancia->id}}" required>
        </div>

        <button type="submit" class="btn btn-primary mr-3">Guardar</button>
        <a href="{{route('lactancia.index')}}" class="btn btn-secondary">Cancelar</a>
    </form>
        ...
        <div class="col-sm-12 col-lg-6">
            <div class="card-body">
                ...
                <div class="alert alert-info">
                    No hay items asociados. Puedes agregar nuevos items utilizando el formulario de carga de archivos.
                </div>
                @endif
            </div>
        </div>
        <div class="col-sm-12 col-lg-6">
            <div class="card p-3">
                <form class="frmItems" enctype="multipart/form-data">
                    ...
                    <button type="button" class="btn btn-primary" id="agregarItemLactancia">Agregar Item</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    $("#agregarItemLactancia").click(function(event) {
